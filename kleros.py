...
class KlerosDispute(Kleros):
    ...
    def __init__(self, dispute_id, contract = None, node_url = None ):
        if contract == None: Kleros.__init__(self, node_url)
        else: self.contract = contract
    ...
    def get_dispute_rounds(self):
        ...
        for i in range(0, len(rounds_raw_data[0])):
            ...
            self.rounds.append(KlerosDisputeRound(self.dispute_id, i, round_data, contract = self.contract))
...
class KlerosDisputeRound(Kleros):
    ...
    def __init__(self, dispute_id, round_id, round_data, contract = None, node_url = None ):
        if contract == None: Kleros.__init__(self, node_url)
        else: self.contract = contract
...
#        self.juror_stake = self.contract.functions.stakeOf(account, subcourtID).call()
#        return self.juror_stake

class KlerosVote(Kleros):
    def __init__(self, dispute_id, appeal, vote_id, contract = None, node_url = None ):
        if contract == None: Kleros.__init__(self, node_url)
        else: self.contract = contract
